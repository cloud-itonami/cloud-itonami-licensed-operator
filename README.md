...
### 許可を「取る」でも「委譲する」でもない第3の経路がある

`:route/principal` と `:route/defer` の2択は、**その業法の中でしか考えていない**。
産廃の2業種には、廃掃法14条の外から14条そのものを外す経路が実在します ——
**小型家電リサイクル法13条**（`:licence/exemptions` の `:kogata-kaden-nintei` /
`:kogata-kaden-jutaku`）。

| | 廃掃法14条 | 小型家電リサイクル法13条 |
|---|---|---|
| 許可権者 | **都道府県知事**（区域ごとに積み上げ） | **主務大臣**（10条の認定 1本） |
| 収運と処分 | 1項と6項で**別々の許可** | 13条1項が**同時に**外す |
| 施設を持たない者 | 名義人になれない | 10条1項括弧書きで**全部委託でも申請可** |
| 委託先 | 6条の2の委託基準に従う（自社は ops 層） | 13条3項で**委託先も許可不要**（自社が実行者のまま） |

13条3項が効くのは、`:route/defer` とは形が違うからです。defer は「許可者を主体に
立てて自社は ops に徹する」。13条3項は**自社が実行者のまま許可義務だけが外れる**
—— 認定計画に10条2項6号の実施者として記載されればよい。

**ただしカタログに収録しても verdict は 1 mm も動きません**（テストが固定）。
認定を受けていない者にこの条文は効かないので、`plan` は依然 `:blocked` を返す。
特例を「読める形で置く」ことと「開ける」ことは別です。
...
現在: **27件（法域×業種）・54の異なるルール** —— JPN 16 + JPN-13 3 + GBR 5 + DEU 3 +
FRA 1 + CAN-ON 2。うち **49 が条文原文の読了**、5 が公式ページの取得で、
